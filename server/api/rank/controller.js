...
_.rankList = async (req) => {
    ...
    total_gender = await MODELS.sequelize.query(query1, {
        raw: true,
        replacements: {depart: depart},
        type: Sequelize.QueryTypes.SELECT
    })
    last_gender = await MODELS.sequelize.query(query2, {
        raw: true,
        replacements: {depart: depart, worship_dt: last_worship},
        type: Sequelize.QueryTypes.SELECT
    })
    total_part = await MODELS.sequelize.query(query3, {
        raw: true,
        replacements: {depart: depart},
        type: Sequelize.QueryTypes.SELECT
    })
    last_part = await MODELS.sequelize.query(query4, {
        raw: true,
        replacements: {depart: depart, worship_dt: last_worship},
        type: Sequelize.QueryTypes.SELECT
    })
