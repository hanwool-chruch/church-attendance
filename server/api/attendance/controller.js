...
_.getAllAttendances = async (req) => {
    ...
    const query = [
        ...
        ' WHERE A.MEMBER_ID = M.MEMBER_ID AND A.DEPART_CD = :depart AND A.WORSHIP_DT > "2022-01-01" ',
...
_.attendanceList = async (req) => {
    ...
    return MODELS.ATTENDANCES.findAll({
        raw: true,
        where: {DEPART_CD: depart, WORSHIP_DT: workshipDT},
        attributes: ['WORSHIP_DT', 'MEMBER_ID', 'TYPE']
    });
}
...
_.getAttendanceDetail = async (req) => {
    ...
    memberListWithPart.forEach(part => {
        ...
        part.memberList.forEach(member => {
            member.attendanceType = 0;
            attendances.forEach(attendance => {
                if (member.MEMBER_ID == attendance.MEMBER_ID) {
                    member.attendanceType = attendance.TYPE;
                    return
                }
            })
        })
...
_.createAttendance = async (req) => {
    ...
    await _.deleteAttendance(req);

    return MODELS.ATTENDANCES.findOrCreate({
        raw: true,
        where: {DEPART_CD: depart, WORSHIP_DT: WORSHIP_DT, MEMBER_ID: MEMBER_ID, TYPE: TYPE}
    });
}
...
_.deleteAttendance = async (req) => {
    ...
    return MODELS.ATTENDANCES.destroy({
        raw: true,
